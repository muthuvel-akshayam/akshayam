  ...
  return (
    <div className="bg-white rounded-lg shadow-md border border-gray-100 overflow-hidden flex flex-col h-full hover:shadow-lg transition-shadow">
      ...
      <div className="p-4 flex flex-col items-center text-center flex-grow">
        <h3 className="text-sm font-bold text-gray-900 mb-0.5 truncate w-full">{profile.name}</h3>
        <span className="text-xs font-bold text-primary mb-2">{profile.displayId || profile.id.slice(0, 8).toUpperCase()}</span>
        
        <div className="text-xs text-slate-600 space-y-1 w-full font-medium">
      ...
      {/* Action Buttons Row */}
      <div className="px-4 pb-4 pt-2 flex gap-2 w-full">
        <Link 
          href={`/profiles/${profile.id}`}
          className="flex-1 bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-semibold py-1.5 px-3 rounded-md text-center transition-colors"
        >
          ப்ரோபைல்
        </Link>
        <Link 
          href={`/profiles/${profile.id}?tab=jathagam`}
          className="flex-1 bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-semibold py-1.5 px-3 rounded-md text-center transition-colors"
        >
          ஜாதகம்
        </Link>
